...
description: "What happened after Part 2 published: the maintainer read it, opened an issue, shipped two fixes in 48 hours, and cited three follow-up issues. Plus an honest updated setup guide — without the package that never existed."
status: "published"
language: "en"
...
If #167 and #170 land together, the pipeline simplifies: one binary for flights and accommodation with duration-bounded search, no fork needed. #168 eliminates the main reason to reach for `serpapi_verified.py`'s `--check-links` mode. None of this is there yet; it is the direction the issues point.
...
**Booking links expire.** Google Hotels ad-click links (`aclk`, `travel/clk`) have a shelf life of hours to a couple of days. Issue #168 will add durable Booking.com fallback links to `trvl` natively. Until it lands: `serpapi_verified.py --check-links` HTTP-validates links and drops the dead ones; it also emits a stable Booking.com deep-link as a fallback.
...
The `fli` fork is required as long as [PR #195](https://github.com/punitarani/fli/pull/195) is open upstream. If it merges, install from the upstream repo instead. If trvl issues #167 and #170 land together, the fli dependency may drop entirely — `trvl dates` would cover the duration-constrained search the fork adds, and the pipeline simplifies to one binary.
