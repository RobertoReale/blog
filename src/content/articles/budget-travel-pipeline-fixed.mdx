...
description: "What happened after Part 2 published: the maintainer read it, opened an issue, shipped two fixes in 48 hours, and opened five follow-up issues — all of which shipped as merged PRs on main. Plus an honest updated setup guide — without the package that never existed."
status: "published"
language: "en"
...
All five PRs merged by end of 2026-06-14, pending the v1.10 release tag.
...
Each PR carries an explicit credit line. PR #173 notes it is *"trvl's native version of his travel-search triage"*; PR #175 notes *"these are his trust rules made native."* PR #176 makes the citation permanent in the tool's official README.

When v1.10 is tagged — check [releases](https://github.com/MikkoParkkola/trvl/releases) — `go install github.com/MikkoParkkola/trvl/cmd/trvl@latest` will pick up all five changes.
...
## The complete setup guide: as of 2026-06-14, trvl v1.9.2
...
- `fli` from the fork (upstream PRs still open — the fork is still required)
- `trvl` v1.9.2+
...
*Note: trvl PR #174, merged on main, adds `--min-duration`/`--max-duration` to `trvl dates` natively. Once v1.10 is released, the fli fork will no longer be needed.*
...
brew upgrade trvl
trvl version   # should print 1.9.2
```

Windows: download `trvl_1.9.2_windows_amd64.tar.gz` from the [v1.9.2 release](https://github.com/MikkoParkkola/trvl/releases/tag/v1.9.2), extract, put `trvl.exe` on your PATH.
...
|---------------|---------|-------|
| Flights — all date combinations in a window | `fli dates BGY-NAP --from 2026-07-22 --to 2026-08-01 --min-duration 5 --max-duration 5` | From the fork. The agent calls `search_dates` via MCP. |
...
| Hotels — per-provider matrix + OTA links | `trvl prices <google_place_id> --checkin 2026-07-30 --checkout 2026-08-04 --currency EUR --format json` | Needs place ID from `trvl hotels --format json`. Best native path for the booking step. |
...
The four-step template from Part 2 is still valid. Two changes from that version: the `.mcp.json` has one fewer server, and the verification in Step 2 uses `trvl serpapi` instead of the `hotel-rates` tool.

A no-code prompt builder is in [`prompt-builder.html`](https://github.com/RobertoReale/travel-search/blob/master/prompt-builder.html) in the `travel-search` repo — open it in a browser, fill in the form, copy the generated prompt. The builder was written against the Part 2 template and still includes the `hotel-rates` step; use the prompt below instead until it's updated.
...
**Booking links expire (fixed in v1.10).** Google Hotels ad-click links (`aclk`, `travel/clk`) have a shelf life of hours to a couple of days. PR #173 is merged on main: it strips `travel/clk` redirects, tags links `stable` vs `expiring`, and attaches a durable Booking.com deep-link as `booking_fallback_url`. This lands in the installable binary with v1.10. Until then: `serpapi_verified.py --check-links` HTTP-validates links and drops the dead ones; it also emits a stable Booking.com deep-link as a fallback.
...
## What the arc felt like

A blog article with a reproducible problem led to a maintainer issuing two releases in 48 hours and opening three follow-up issues citing the findings by name. That is not exceptional — it is what open source looks like when it works. The maintainer gets accurate, reproducible data. The reporter gets a tool that's more trustworthy. The issues document the broader problem space for anyone who hits the same thing later.

This was a first open source contribution arc: not a code PR, but a structured field report that became a regression test and three open issues. The same tool is now safer for everyone who uses it in an agent pipeline. The medium of contribution turned out to be the article itself.
...
The `fli` fork is required as long as [PR #195](https://github.com/punitarani/fli/pull/195) is open upstream. If it merges, install from the upstream repo instead. trvl PR #174 has merged on main, adding `--min-duration`/`--max-duration` to `trvl dates` natively. Once v1.10 is released, the pipeline can consolidate to one binary for both flights and accommodation, and the fli fork will no longer be needed.
