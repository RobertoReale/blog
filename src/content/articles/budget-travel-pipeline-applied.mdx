...
The template below works for any departure city, destination list, and date window. Replace the bracketed values before running. If you'd rather not hand-edit the brackets, [`travel-search` ships a no-code prompt builder](https://github.com/RobertoReale/travel-search/blob/master/prompt-builder.html) — a single self-contained HTML page that turns a form into this exact prompt.
